...
namespace Wiggy
{
  public class UnitSpawnSystem : ECSSystem
  {
    ...
    public void Start(Wiggy.registry ecs, UnitSpawnSystemInit data)
    {
      ...
      var generated_map_parent = GameObject.Find("generated_map_parent");
      foreach (Transform child in generated_map_parent.transform)
        Object.DestroyImmediate(child.gameObject);

      var generated_obj_parent = new GameObject("generated_objects_parent");
      generated_obj_parent.transform.SetParent(generated_map_parent.transform);

      Spawn(ecs, map, EntityType.tile_type_wall, "Walls", new Optional<GameObject>(data.wall_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.actor_player, "Player", new Optional<GameObject>(data.player_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.actor_enemy, "Enemy", new Optional<GameObject>(data.enemy_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.actor_barrel, "Bomb", new Optional<GameObject>(data.barrel_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.tile_type_trap, "Trap", new Optional<GameObject>(data.trap_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.keycard, "Keycard", new Optional<GameObject>(data.keycard_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.tile_type_door, "Door", new Optional<GameObject>(data.door_prefab), new Optional<GameObject>(generated_obj_parent));
      Spawn(ecs, map, EntityType.tile_type_exit, "Exit", new Optional<GameObject>(data.exit_prefab), new Optional<GameObject>(generated_obj_parent));
