...
namespace Wiggy
{
  ...
  public class poisson
  {
    ...
    public poisson(float width, float height, float radius)
    {
    ...
    private Vector2 RandomFirstSample()
    {
      return new Vector2(Random.value * rect.width, Random.value * rect.height);
    }

    /// Return a lazy sequence of samples. You typically want to call this in a foreach loop, like so:
    ///   foreach (Vector2 sample in sampler.Samples()) { ... }
    public IEnumerable<Vector2> Samples(int seed, Vector2 first_sample)
    {
      Random.InitState(seed);

      // First sample is choosen randomly
      yield return AddSample(first_sample);
